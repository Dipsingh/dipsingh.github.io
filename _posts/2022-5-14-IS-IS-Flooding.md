...
Looking at the LSP flood events for the LSP generated by `sea1_b1_t1_r1`, we see:

- T0: `sea1-b2-t1-r1` -> `pdx1-b2-t1-r1`
- T1 - T4: `pdx1-b2-t1-r1` -> `pdx1-b2-t2-r[1234]`. At this point all the Routers at T2 layers have an updated LSP Update.

![Time events1](/images/post11/tzero.png "LSP event")

- T5 - T7: `pdx1-b2-t2-r3` -> `pdx1-b2-t1-r[234]`. We see `pdx1-b2-t2-r3` sends an update to `pdx1-b2-t1-r[234]`.
We can see that `pdx1-b2-t2-r3` didn't send it back to `pdx1-b2-t1-r1`. 
...
to use a bash script on the box to periodically enable/disable interface at regular intervals
...
do
date
echo 'show interfaces ethernet 5 | include Ethernet5'
FastCli -p 15 -c 'show interfaces ethernet 5'
echo ''
echo 'config shut ethernet5'
FastCli -p 15 -c $'conf\n interface ethernet 5\n shutdown'
sleep 2
echo 'config un shut ethernet5'
FastCli -p 15 -c $'conf\n interface ethernet 5\n no shutdown'
echo ''
echo '-----------------------------------------------------'
sleep 10
...
[admin@EOS1 ~]$
[admin@EOS1 ~]$ cd /mnt/flash
[admin@EOS1 flash]$ vi collect-data.sh
[admin@EOS1 flash]$ sudo chmod 755 collect-data.sh
[admin@EOS1 flash]$
EOS1(config)#daemon collect-data
EOS1(config-daemon-collect-data)#exec /mnt/flash/collect-data.sh
EOS1(config-daemon-collect-data)#no shutdown
EOS1(config-daemon-collect-data)#end
EOS1#
```

Code to collect the output of `show isis database level-2` from any node.
